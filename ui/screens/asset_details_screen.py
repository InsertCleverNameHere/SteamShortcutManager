...
class DownloadWorker(QObject):
    ...
    def run(self):
        ...
        success, message = download_assets(
            self.steam_id, self.local_id, self.grid_dir, self.force, 
...
class AssetDetailsScreen(QWidget):
    ...
    def _build_ui(self):
        ...
        self.suggestion_thumb.setStyleSheet("background: #000; border-right: 1px solid #2e3340;")
        self.suggestion_layout.addWidget(self.suggestion_thumb)
        
        self.suggestion_text = QLabel("")
        self.suggestion_text.setStyleSheet(f"color: {PALETTE['accent']}; font-size: 11px; font-weight: 800; border: none; background: transparent;")
    ...
    def _on_inject_clicked(self, auto_id=None):
        ...
        self._worker.status_update.connect(self.status_label.setText) # Update text in real-time
    ...
    def _on_download_finished(self, success, message):
        ...
        if success:
            ...
            self.load_assets(self._current_name, self._current_shortcuts_path, self._current_appid)
        else:
    ...
    def load_assets(self, game_name, shortcuts_path, appid):
        ...
        positions = {"capsule": (0, 0), "header": (0, 1), "hero": (1, 0), "logo": (1, 1), "json": (2, 0)}
        ...
        for key, (exists, path) in status.items():
            ...
            label.setStyleSheet(f"font-size: 12px; font-weight: 900; letter-spacing: 1.5px; color: {PALETTE['accent']}; background: transparent;")
